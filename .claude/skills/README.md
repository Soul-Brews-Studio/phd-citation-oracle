...
| Skill | What it does |
|-------|--------------|
| [`lit-review`](lit-review/SKILL.md) | **Run the working loop**, starting from whatever you actually have — one reference, a list, a full AI report, or **only a topic**. For a topic it routes you into two *independent* retrievals and **cross-checks them against each other** before anything becomes a card: papers found by only one source are where fabrications live. Then verify against Crossref → re-index → read the corpus as a map → hunt the next gap. The step people skip is *verify*, and on this corpus verifying 62 inherited cards found **18 errors in 14 of them**. Also covers threshold tuning as a way of *reading* the map, and why a search finding nothing is not proof that nothing exists. Use the three skills below inside this loop. |
...
| [`research-ingest`](research-ingest/SKILL.md) | **Absorb** a deep-research report produced elsewhere (Gemini/ChatGPT/Perplexity) into the vault and the local vector store: file it verbatim with provenance, extract the bibliography, **verify every supplied DOI against Crossref before it touches a card**, reconcile against the existing corpus (enrich / create / hold), re-index, then prove it searchable. Completes the loop with `gemini-deep-research` and `paper-card`. |
| [`research-harvest`](research-harvest/SKILL.md) | **Convert** a filed report into work products: the decision it supports (with the conditions that flip it), a thesis-ready comparison table that keeps measurement conditions attached to every number, card enrichments, a verification queue built from the report's own admissions, follow-up briefs for its gaps (classified as search-failure / real-absence / paywall), and the conflicts a human must adjudicate. |
