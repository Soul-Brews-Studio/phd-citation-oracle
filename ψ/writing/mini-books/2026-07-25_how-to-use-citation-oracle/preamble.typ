...
#set text(font: ("IBM Plex Sans Thai Looped", "Sarabun", "Apple Color Emoji"), lang: "th")
...
#v(2em)
#align(center, text(size: 32pt, weight: "bold", fill: rgb("#1a1a2e"))[คู่มือใช้ Citation Oracle])
#v(1.2em)
#align(center, text(size: 13pt, fill: luma(100))[จาก 62 การ์ด markdown ไปเป็น .bib ที่อ้างอิงได้จริง — และ 18 ข้อผิดพลาดที่เจอตอนทำ])    // SUBTITLE = 1-2 lines: what + the honest angle
#v(3em)
#align(center, text(size: 12pt, weight: "bold", fill: rgb("#c0392b"))[
  Citation Oracle ★ (AI, ไม่ใช่คน) — จาก ณัฐ วีระวรรณ์
])
#v(0.5em)
#align(center, text(size: 10pt, fill: luma(140))[25 กรกฎาคม 2026 · ทุกคำสั่งในเล่มนี้รันจริงบน m5 (Apple M5 Max) · commit c01b2f5 · mini-book])    // PROOF_NOTE e.g. พิสูจน์ด้วย commit จริง
#v(1fr)
#line(length: 100%, stroke: 3pt + rgb("#c0392b"))
...
// `par(spacing:)` is what actually separates paragraphs. `block(spacing:)` does
// NOT — typst compiles it without error or warning while the paragraphs run
// together (measured on typst 0.15.1: same file, block→71px of vertical extent,
// par→110px). Verified 2026-07-25 after pendulum-oracle hit it on a 90pp book.
#set par(leading: 1.34em, spacing: 1.7em, justify: false)
