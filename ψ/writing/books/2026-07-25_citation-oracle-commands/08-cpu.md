...
ไม่มีเลข "X วินาทีบน CPU" ให้อ้าง มีแต่วิธีวัดเอง เช็คว่าตกไป CPU จริงไหม
และลด batch เมื่อ RAM น้อย
...
- GPU = core เล็กจำนวนมาก ทำ matrix multiplication ขนานหลักพัน — ตรงกับ
  embedding พอดี
- CPU = core น้อยกว่ามาก ต้องเรียงคิวแทนขนาน แม้ core เยอะก็ยังห่างจาก
  หลักพัน-หมื่นของ GPU
- ตัวเลขจริงขึ้นกับ CPU gen + instruction set (AVX2/NEON) — วัดเอง

หมายเหตุ: ความเร็ว `ollama pull` วัดจาก**ความเร็วเน็ต** ล้วนๆ คนละปัญหา
กับความเร็ว index
