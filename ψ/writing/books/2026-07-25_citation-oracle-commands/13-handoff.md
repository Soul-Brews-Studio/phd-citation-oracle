...
ข้อ paywall ไม่ใช่เห็นหน้าว่างเปล่า — รายละเอียดลึกกว่า abstract จะถูกเดาจาก pattern ของ paper แนวเดียวกัน แล้วตอบมาอย่างมั่นใจเหมือนอ่านจริง

เช่นข้อสาม — แทนที่จะขอ "คำนวณ CCC ระหว่างเซนเซอร์กับ AOD" ให้ขอ "หา metric ที่ literature ใช้วัด agreement ระหว่างเซนเซอร์ราคาถูกกับ reference monitor" — ตอบได้จากสิ่งที่ paper รายงานไว้แล้ว ไม่ต้องคำนวณใหม่

## 13.2 ดึง context จาก vault ไปด้วย — `search --json`

tool อ่านไฟล์เราไม่ได้ — ต้องดึง context ใส่ brief เอง ไม่งั้นรายงานเสียพื้นที่เล่าซ้ำสิ่งที่มีอยู่แล้ว
...
`citekey` + `title` + `journal` วางเป็น bullet list ใน "สิ่งที่เรารู้แล้ว" — `id` ที่คืนคือ `paper:<citekey>` ไม่ใช่เลขจาก frontmatter (เช่น `2.3.4`) อย่าเอาไปเขียนเป็นเลขอ้างอิง
...
สิบหกใบนี้คือของที่ tool จะไม่เสียเวลาหาซ้ำ เหลือแต่สิ่งที่ยังไม่มี (MODIS/MAIAC, VIIRS, TROPOMI, Himawari, MISR)
...
ถ้าเฉลี่ยเป็น "underestimate ปานกลาง" คำตอบผิดทั้งสองช่วง — ผิดทิศที่ AOD ต่ำกว่า 0.5 (จริงคือ overestimate) และผิดขนาดที่ AOD สูงกว่า 1.0 ช่วง burning season ที่ thesis ต้องใช้เทียบข้อมูลจริง
...
เคส Chen X. อันตรายเพราะ DOI "ถูก" — resolve เจอ paper จริง แต่ชื่อคนกับชื่อเรื่องไม่ตรงกับตัว paper เช็ค resolve อย่างเดียวไม่พอ ต้องเช็ค title/author กับ Crossref ด้วย (บทที่ 14)

---

brief ที่ทำตามทุกข้อในบทนี้ก็ยังไม่ได้แปลว่ารายงานที่ได้กลับมาจะถูกทั้งหมด

brief คุมได้แค่สิ่งที่ *ขอ* ไม่ได้คุมสิ่งที่ *ได้กลับมาจริง* — tool แต่งชื่อ paper ผิดได้ วางข้าง DOI ที่ถูก 100% จนตรวจด้วยตาเปล่าไม่เจอ เชื่อไม่ได้จนผ่านตรวจอีกรอบ — บทที่ 14 พาไปดูวิธี
