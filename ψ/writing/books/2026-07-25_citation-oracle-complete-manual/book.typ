...
#set text(font: ("Laksaman", "Norasi"), size: 12pt, lang: "th")
...
#align(center, text(size: 13pt, fill: luma(90))[
  เปลี่ยนกองเอกสารอ้างอิง ให้เป็น `.bib` ที่เชื่อถือได้ \
  ทุกเครื่อง ทุกที่ ทุกงบ — corpus อะไรก็ใช้ได้
])
#v(2.5em)
...
#align(center, text(size: 9.5pt, fill: luma(140))[
  25 กรกฎาคม 2026 · 15 บท · ทุกคำสั่งในเล่มรันจริงแล้ว \
  ตัวอย่างจาก corpus จริง 62 paper — DOI ยืนยัน 61/62 · เจอ error 18 จุด
])
#v(1fr)
...
#show heading.where(level: 2): it => {
  ...
  v(1.2em); block(it); v(0.4em)
}

#show heading.where(level: 3): it => {
  set text(size: 12pt, weight: "bold", fill: rgb("#34495e"))
  v(0.8em); block(it); v(0.2em)
}

#show raw.where(block: true): it => {
  set text(font: "Fira Code", size: 8.5pt)
  block(fill: rgb("#f6f8fa"), stroke: 0.5pt + luma(200),
    inset: 12pt, radius: 4pt, width: 100%, it)
}
